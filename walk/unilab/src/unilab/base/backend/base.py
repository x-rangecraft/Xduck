...
class SimBackend(abc.ABC):
    ...
    def get_dof_frictionloss(self) -> np.ndarray:
        """Return the backend per-DOF Coulomb-friction table."""
    ...
    def enable_dof_force_cache(self, enabled: bool = True) -> None:
        """Enable final-solve DOF force capture for stateful actuator models."""
        if enabled:
            raise NotImplementedError(
                f"{self.__class__.__name__} does not support DOF force capture"
            )

    def get_dof_force_components(
        self,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Return cached bias, constraint, actuator and friction DOF forces."""
        raise NotImplementedError(
            f"{self.__class__.__name__} does not support DOF force capture"
        )
    ...
    def sample_site_clearance(
        ...
        *,
        ...
        """Sample group-0 vertical ray clearance at named site IDs."""
        raise NotImplementedError(
            f"{self.__class__.__name__} does not support site clearance rays"
        )

    def set_runtime_dof_properties(
        self, *, frictionloss: np.ndarray, damping: np.ndarray
    ) -> None:
        """Patch per-environment DOF friction fields without resetting state."""
        raise NotImplementedError(
            f"{self.__class__.__name__} does not support runtime DOF field updates"
        )
