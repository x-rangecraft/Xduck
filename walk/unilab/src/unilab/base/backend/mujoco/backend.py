...
class MuJoCoBackend(SimBackend):
    ...
    def __init__(
        ...
        actuator_force_range: tuple[float, float] | None = None,
        actuator_ctrl_range: tuple[float, float] | None = None,
        dof_armature: float | np.ndarray | None = None,
        dof_damping: float | np.ndarray | None = None,
        dof_frictionloss: float | np.ndarray | None = None,
        dof_friction_solref: tuple[float, float] | None = None,
        dof_friction_solimp: tuple[float, float, float, float, float] | None = None,
        ...
        self._actuator_force_range = actuator_force_range
        self._actuator_ctrl_range = actuator_ctrl_range
        self._configured_dof_armature = dof_armature
        self._configured_dof_damping = dof_damping
        self._configured_dof_frictionloss = dof_frictionloss
        self._dof_friction_solref = dof_friction_solref
        self._dof_friction_solimp = dof_friction_solimp
        ...
        self._capture_dof_forces = False
        self._dof_force_components = np.zeros((num_envs, 4, self._model.nv), dtype=self._np_dtype)
    ...
    def _configure_model(self, model: mujoco.MjModel) -> None:
        ...
        if self._actuator_force_range is not None:
            low, high = (float(value) for value in self._actuator_force_range)
            if not low < high:
                raise ValueError("actuator_force_range must satisfy low < high")
            model.actuator_forcelimited[:] = 1
            model.actuator_forcerange[:, 0] = low
            model.actuator_forcerange[:, 1] = high
        if self._actuator_ctrl_range is not None:
            low, high = (float(value) for value in self._actuator_ctrl_range)
            if not low < high:
                raise ValueError("actuator_ctrl_range must satisfy low < high")
            model.actuator_ctrllimited[:] = 1
            model.actuator_ctrlrange[:, 0] = low
            model.actuator_ctrlrange[:, 1] = high
        if self._configured_dof_armature is not None:
            values = np.asarray(self._configured_dof_armature, dtype=np.float64)
            if values.ndim == 0:
                model.dof_armature[:] = float(values)
            else:
                if values.shape != model.dof_armature.shape:
                    raise ValueError(
                        "dof_armature must be scalar or have shape "
                        f"{model.dof_armature.shape}, got {values.shape}"
                    )
                model.dof_armature[:] = values
        for field_name, configured in (
            ("dof_damping", self._configured_dof_damping),
            ("dof_frictionloss", self._configured_dof_frictionloss),
        ):
            ...
            if values.ndim == 0:
                ...
                raise ValueError(
                    f"{field_name} must be scalar or have shape {target.shape}, got {values.shape}"
                )
        # BAM's joint-friction solver settings are a cold-path model property,
        # not an actuator torque.  Apply them before the batch pool is built.
        if self._dof_friction_solref is not None:
            model.dof_solref[
                model.jnt_dofadr[model.jnt_type == int(mujoco.mjtJoint.mjJNT_HINGE)]
            ] = self._dof_friction_solref
        if self._dof_friction_solimp is not None:
            model.dof_solimp[
                model.jnt_dofadr[model.jnt_type == int(mujoco.mjtJoint.mjJNT_HINGE)]
            ] = self._dof_friction_solimp
    ...
    def enable_dof_force_cache(self, enabled: bool = True) -> None:
        self._capture_dof_forces = bool(enabled)
        if not enabled:
            self._dof_force_components.fill(0.0)

    def get_dof_force_components(
        self,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        return tuple(self._dof_force_components[:, index] for index in range(4))  # type: ignore[return-value]
    ...
    def sample_site_clearance(
        ...
        *,
        ...
        if self._pool is None:
            raise RuntimeError("site clearance requires a materialized backend")
        return np.asarray(
            self._pool.sample_site_clearance(
                self._physics_state,
                site_ids,
                radius=radius,
    ...
    def set_runtime_dof_properties(self, *, frictionloss: np.ndarray, damping: np.ndarray) -> None:
        if self._pool is None:
            raise RuntimeError("runtime DOF properties require a materialized backend")
        expected = (self._num_envs, self._model.nv)
        frictionloss = np.asarray(frictionloss, dtype=np.float64)
        damping = np.asarray(damping, dtype=np.float64)
        if frictionloss.shape != expected:
            raise ValueError(f"frictionloss must have shape {expected}, got {frictionloss.shape}")
        if damping.shape != expected:
            raise ValueError(f"damping must have shape {expected}, got {damping.shape}")
        self._pool.patch_runtime_fields({"dof_frictionloss": frictionloss, "dof_damping": damping})
    ...
    def step(self, ctrl: np.ndarray, nsteps: int = 1) -> dict | None:
        ...
        t0 = time.perf_counter()
        step_out = self._pool.step(  # type: ignore[union-attr]
            self._physics_state,
            nstep=nsteps,
            ...
            return_dof_forces=self._capture_dof_forces,
        )
        if self._capture_dof_forces:
            state_np, sensor_np, dof_forces = step_out
            self._dof_force_components[:] = dof_forces.astype(self._np_dtype)
        else:
            state_np, sensor_np = step_out
    ...
    def _apply_body_linear_velocity_delta(
        ...
        expected = (self._num_envs, body_ids_np.size, 3)
        if delta.shape != expected:
            raise ValueError(f"body velocity delta must have shape {expected}, got {delta.shape}")
        if body_ids_np.size != 1 or int(body_ids_np[0]) != self._base_body_id:
            raise NotImplementedError(
                "MuJoCo interval velocity delta currently supports only the free-base body"
            )
        self._physics_state[:, self._idx_qvel : self._idx_qvel + 3] += delta[:, 0, :]
    ...
    def _translate_reset_randomization(
        ...
        if randomization.dof_frictionloss is not None:
            translated["dof_frictionloss"] = self._coerce_reset_field(
