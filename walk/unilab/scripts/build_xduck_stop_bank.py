...
def main():
    ...
    for key in ("scene", "reference_keyframe", "action_offset_low", "action_offset_high"):
        if key in saved["env"]:
            o[key] = saved["env"][key]
    o["noise_config"]["level"] = 0.0
    o["domain_rand"].update(
        velocity_pushes=False,
        randomize_dof_armature=False,
        foot_friction_range=[1.0, 1.0],
        mass_inertia_scale_range=[1.0, 1.0],
        imu_max_angle_deg=0.0,
    )
    for k in ["com_range_stages", "head_com_range_stages"]:
        o["curriculum"][k] = [{"step": 0, "value": 0.0}]
    env = E.create_env(cfg, num_envs=16, env_cfg_override=o)
    try:
        env._encoder_bias.fill(0.0)
        rl = E._algo_config_dict(cfg)
