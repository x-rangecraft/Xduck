    ...
    def compute_torque(
        ...
        *,
        ...
        if self.friction_mode != "smooth_stribeck":
            ...
            coast = self.coast_state
        drive_effort = physical["torque_scale"] * self.effort - viscous * coast * v
        if self.friction_mode == "smooth_stribeck":
            fc = np.where(v >= 0, physical["coulomb"], physical["coulomb_negative"]) * coast
            ...
            fc = np.where(required >= 0, physical["coulomb"], physical["coulomb_negative"]) * coast
