...
| STM32 电机命令与 CAN 双向短时采集 | 标准固件默认编入 `Bsp/Src/can_capture.c`；电脑端入口 `60-tools/capture-stm32-can.py`；采集文件存放 `50-logs/test/motor/gf43x40-10-i2rt/data/can-capture/`。使用匹配的合并镜像 ELF 导出 |
