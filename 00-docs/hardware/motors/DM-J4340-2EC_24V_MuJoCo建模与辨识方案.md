# DM-J4340-2EC 24 V 电机 MuJoCo 建模、实机辨识与域随机化方案

## 1. 文档目的

本文定义 DM-J4340-2EC 24 V 关节电机在机器人项目中的统一建模方法，供机械、嵌入式、控制、仿真和强化学习团队共同使用。
...
| 项目 | 统一值 | 工程含义 |
|---|---:|---|
| 电机型号 | GF43X40-10 / DM-J4340-10 24 V | 40:1、362 g 版本 |
| 标称母线电压 | 24 V | 默认仿真电压 |
...
```text
j4340_24v_torque_current.csv
j4340_24v_torque_speed.csv
j4340_24v_torque_efficiency.csv
j4340_24v_torque_input_power.csv
j4340_24v_temperature_rated.csv
```
...
`dm_j4340_comm/data/processed/j4340_empty_fit.json`。
...
`j4340_empty_fit.json.datasheet_gf43x40_10_24V.performance_table`，包含电压、母线电流、
...
```python
def step_j4340(action, state, params, dt):
...
UniLab 将域随机化分为 init、reset 和 interval 三种生命周期。J4340 的电机参数主要在 reset 时采样，在每个物理步的 pre-step control callback 中使用。
...
| 3 | 拟合空载增益、噪声、延迟、时间常数、摩擦和惯量 | `j4340_empty_fit.json` |
| 4 | 实现 MuJoCo J4340 torque actuator | 执行器代码和单元测试 |
| 5 | 完成单关节仿真—空载实机对比 | 空载验证报告 |
| 6 | 机器人悬空采集 14 关节带载数据 | 真机辨识数据集 |
| 7 | 拟合每关节修正和力矩—速度曲面 | `j4340_joint_fit.json` |
