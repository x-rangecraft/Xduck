...
/// DM CAN wire position range. Per-motor calibrated bounds are enforced on STM32;
/// this outer guard rejects targets that cannot be represented on the motor wire.
pub const ACTUATOR_MIN: f64 = -12.5;
pub const ACTUATOR_MAX: f64 = 12.5;
...
impl<T: RobotIo> Safety<T> {
    ...
    pub fn motor_torque_enabled(&self) -> Option<bool> {
        self.io.motor_torque_enabled()
    }

    pub fn motor_control_error(&self) -> Option<&'static str> {
    ...
    pub fn position_limits_ready(&self) -> Option<bool> { self.io.position_limits_ready() }
    pub fn mark_motor_zero(&mut self, motor_id: u8) -> Result<(), IoError> {
        self.io.mark_motor_zero(motor_id)
    }
