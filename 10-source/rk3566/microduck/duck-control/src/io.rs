...
//!
//! [`Sensors`] carries joints and IMU as one control-tick sample. The STM32 USB gateway
//! places both in one DMUSB-v3 state frame, so they share the same 50 Hz timestamp.
...
pub struct Sensors {
    ...
    /// Age of the most recent motor feedback according to the STM32 clock, in ms.
    pub motor_feedback_age_ms: [u32; NUM_JOINTS],
    /// Fused roll, pitch and yaw in radians, as reported by the STM32 IMU block.
    pub attitude_rpy: [f64; 3],
    pub imu: ImuData,
}
...
impl Default for Sensors {
    fn default() -> Self {
        Self {
            ...
            motor_torques_nm: [0.0; NUM_JOINTS],
            motor_flags: [0; NUM_JOINTS],
            motor_temperatures_c: [0.0; NUM_JOINTS],
            motor_feedback_age_ms: [0; NUM_JOINTS],
            attitude_rpy: [0.0; 3],
            imu: ImuData::default(),
        }
    }
...
pub trait RobotIo {
    ...
    fn position_limits_ready(&self) -> Option<bool> { None }
    fn mark_motor_zero(&mut self, _motor_id: u8) -> Result<()> {
        Err(IoError::Bus("此后端不支持电机零位标定".into()))
    }
    ...
    /// A transport-specific reason why a new torque-on request cannot be served.
    /// This never prevents torque-off, and fakes require no hardware handshake.
    fn motor_control_error(&self) -> Option<&'static str> { None }

    /// Actual hardware feedback, when this backend reports motor enable bits.
    fn motor_torque_enabled(&self) -> Option<bool> { None }
...
impl RobotIo for FakeIo {
    fn set_position_limits(&mut self, _: &[MotorPositionLimit]) -> Result<()> { Ok(()) }
    fn read(&mut self) -> Result<Sensors> {
