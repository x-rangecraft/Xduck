...
struct Args {
    ...
    /// STM32 motor-gateway port override.
    #[arg(long)]
    port: Option<String>,

    /// Independent IMU UART override.
    #[arg(long)]
...
struct RobotState {
    ...
    motor_control_error: ArcSwapOption<String>,
    /// Retain a queued power action's outcome until the next operator power request.
    power_error: ArcSwapOption<String>,
    ...
    /// reading the startup pose. Non-zero means the loop is waiting for the STM32, either
    /// at process startup or after dropping a failed USB handle for reconnection.
    ...
    shutdown: AtomicBool,
    /// Latest discrete request from any client, so another client (notably the web console
    /// watching `padd`) can report it. Continuous intents never enter this slot.
    operator_event: ArcSwapOption<proto::OperatorEvent>,
    operator_event_seq: AtomicU64,
    /// Suppress the 50 Hz repeated `wheee hold=true` notifications from becoming 50 feedback
    /// events per second. The start and release edges are still published.
    wheee_reported: AtomicBool,
...
impl RobotState {
    fn new(params: &Params, force_unhealthy: bool, force_busy: bool) -> Self {
        Self {
            ...
            operator_event: ArcSwapOption::empty(),
            operator_event_seq: AtomicU64::new(0),
            wheee_reported: AtomicBool::new(false),
    ...
    fn publish_operator_event(
        &self,
        kind: &'static str,
        action: &'static str,
        result: &proto::IntentResult,
    ) {
        let sequence = self.operator_event_seq.fetch_add(1, Ordering::Relaxed) + 1;
        self.operator_event.store(Some(Arc::new(proto::OperatorEvent {
            sequence,
    ...
    fn health(&self) -> proto::HealthResult {
        ...
        if waiting > 0 {
            // Degraded, not unhealthy: USB wiring or an unpowered bench board cannot be
            // repaired by rolling the installed release back. This applies equally during
            // initial bring-up and while reconnecting after a cable was removed.
            let reason = if ticks == 0 {
                format!(
                    "STM32 motor/IMU gateway unavailable after {waiting} attempts; \
                     check power, /dev/ttyACM0, and USB wiring"
                )
            } else {
                format!(
                    "STM32 motor/IMU USB link is down; reconnecting (attempt {waiting}); \
...
async fn main() -> ExitCode {
    ...
    if let Some(imu_port) = args.imu_port.clone() {
        params.bus.imu_port = imu_port;
    }
    if args.no_policy {
...
fn run_init(params: &Params, duration: Duration) -> ExitCode {
    let mut io = match duck_control::bus::DynamixelIo::open_with_imu(
        &params.bus.port,
        &params.bus.imu_port,
    ) {
        Ok(io) => io,
    ...
    if let Err(e) = io.check_registers() {
        tracing::error!(error = %e, "STM32 motor/IMU gateway startup check failed");
        return ExitCode::FAILURE;
    }
    let calibration = calibration::Calibration::default();
    calibration.load(PathBuf::from(calibration::PATH));
    let config = calibration.config();
    if let Some(error) = calibration.status().error.or_else(|| config.home_error()) {
        tracing::error!(%error, "invalid motor calibration");
        return ExitCode::FAILURE;
    }
    if let Err(e) = io.set_position_limits(&config.wire_limits()) {
        tracing::error!(error = %e, "cannot synchronize motor limits");
        return ExitCode::FAILURE;
    }
    ...
    if let Err(e) = io.interpolate_to(&config.home, duration, Duration::from_millis(20)) {
...
fn spawn_control_thread(
    ...
    let fake = args.fake;
    if !fake { state.calibration.load(PathBuf::from(calibration::PATH)); }
    state.motor_control_error.store((!fake).then(|| Arc::new(
        "尚未收到 STM32 电机状态".to_owned(),
    )));
    let port = params.bus.port.clone();
    let imu_port = params.bus.imu_port.clone();
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            runtime.block_on(async move {
                loop {
                    let Some(io) = open_bus_waiting(&port, &imu_port, &state).await else {
                        break;
                    };
                    match control_loop(
                        io,
                    ...
                    .await
                    {
                        ControlLoopExit::Shutdown => break,
                        ControlLoopExit::BusLost => tracing::warn!(
                            port,
                            "STM32 USB handle closed; waiting for the device to reconnect"
                        ),
                    }
                }
            });
...
async fn open_bus_waiting(port: &str, imu_port: &str, state: &RobotState) -> Option<BusIo> {
    ...
    while !state.shutdown.load(Ordering::Relaxed) {
        ...
        if let Some(io) = open_bus(port, imu_port, attempt) {
...
#[cfg(target_os = "linux")]
fn open_bus(port: &str, imu_port: &str, attempt: u32) -> Option<BusIo> {
    ...
    let mut io = match duck_control::bus::DynamixelIo::open_with_imu(port, imu_port) {
        Ok(io) => io,
        Err(e) => {
            if loud {
    ...
    match io.check_registers() {
        Ok(0) => tracing::info!("STM32 motor/IMU gateway is ready"),
        Ok(n) => tracing::warn!(corrected = n, "hardware startup settings corrected"),
        Err(e) => {
            if loud {
                tracing::error!(
                    error = %e,
                    attempt,
                    "STM32 motor/IMU gateway startup check failed; waiting"
                );
            }
            return None;
        }
...
#[cfg(not(target_os = "linux"))]
fn open_bus(_port: &str, _imu_port: &str, _attempt: u32) -> Option<BusIo> {
...
fn build_controller(
    ...
    }
    {
        ...
        state.models.resolve(&mut paths);
        if let Some(error) = state.models.load_error() {
...
/// Prime the sit-to-stand session immediately before enabling torque.
///
/// Loading already warms every model with a zero observation. This second pass uses the
/// current seated observation and, crucially, happens while the motors are still off. It
/// prevents lazy runtime work from starving STM32's 100 ms host-command watchdog after the
/// enable acknowledgement.
fn prepare_seated_rise(
    controller: &mut Option<Controller>,
    seated_boot: bool,
    sensors: &duck_control::Sensors,
    dt: f64,
) -> Result<bool, duck_control::policy::PolicyError> {
    if !seated_boot || !controller.as_ref().is_some_and(Controller::has_sitstand) {
        return Ok(false);
    }
    controller
        .as_mut()
        .expect("checked above")
        .prepare_boot_rise(sensors, dt)?;
    Ok(true)
}
...
async fn control_loop<T: RobotIo>(
    ...
    // startup-seat decision is stale: the robot is now at home, so a later policy enable must
    // not run the sitstand rise a second time.
    let mut init_homing = false;
    ...
    while !state.shutdown.load(Ordering::Relaxed) {
        ...
        let fresh = match safety.read() {
            ...
            Err(e) => {
                state.motor_control_error.store(Some(Arc::new(
                    "STM32 电机状态读取失败，请检查 USB 连接".to_owned(),
                )));
                ...
                let reconnect_after = state.max_consecutive_errors.max(COAST_TICKS + 1);
                if n >= reconnect_after {
                    // Logical safety is immediate: forget all motion intent and require a
                    // fresh explicit enable after reconnection. Physical safety does not
                    // depend on this process reaching the disconnected USB device: the STM32
                    // firmware independently disables every motor route after 100 ms without
                    // a host command.
                    intents.stop();
                    intents.set_enabled(false);
                    state.moving.store(false, Ordering::Relaxed);
                    state.homed.store(false, Ordering::Relaxed);
        ...
        if fresh.is_some() {
            state.motor_control_error.store(safety.motor_control_error()
                .map(|reason| Arc::new(reason.to_owned())));
            // A lost enable bit or a motor fault invalidates the remembered bring-up.
            // Require a new explicit operator request after the hardware recovers.
            if bringup != Bringup::Limp && (safety.motor_control_error().is_some()
                || safety.motor_torque_enabled() == Some(false)) {
                tracing::warn!("motor feedback no longer ready; releasing joints and clearing policy");
        ...
        if let Some(fresh) = fresh.as_ref() {
            ...
            if safety.imu_ready() {
                odometry.update_timed(&fresh.positions, fresh.imu.quat, period.as_secs_f64());
            }
        }
        ...
        match power_request {
            Some(intents::PowerRequest::Init) => match (bringup, sensors.as_ref()) {
                ...
                (Bringup::Limp, Some(sensors)) => match safety.set_torque(true) {
                    ...
                    Err(e) => {
                        state.power_error.store(Some(Arc::new(e.to_string())));
                        intents.set_enabled(false);
                        tracing::warn!(error = %e, "cannot enable torque");
                    }
                },
            ...
            Some(intents::PowerRequest::Relax) => {
                ...
                match safety.set_torque(false) {
                    Ok(()) => {
                        ...
                        // Next bring-up starts from wherever the robot actually is.
        ...
        if let (Bringup::Limp, true, true, Some(sensors)) = (
            ...
            match prepare_seated_rise(&mut controller, seated_boot, sensors, dt) {
                Err(e) => {
                    state
                        .power_error
                        .store(Some(Arc::new(format!("起立准备失败：{e}"))));
                    intents.set_enabled(false);
                    tracing::warn!(error = %e, "cannot prepare seated-rise inference");
                }
                Ok(rise_prepared) => match safety.set_torque(true) {
                    Ok(()) => {
                        if rise_prepared {
                    ...
                    // A failed hardware enable needs a new explicit request, not 50 retries/s.
                    Err(e) => {
                        if rise_prepared
                            && let Some(controller) = controller.as_mut()
                        {
                            controller.cancel_rise();
                        }
                        state.power_error.store(Some(Arc::new(e.to_string())));
                        intents.set_enabled(false);
        ...
        if bringup == Bringup::Limp || powered_off || in_limp_fall {
            init_homing = false;
        }
        ...
        {
            if init_homing {
                // `robot.init` always reaches home by interpolation. A later policy enable now
                // starts from a standing robot rather than replaying the boot-time seated rise.
                seated_boot = false;
                init_homing = false;
            }
            ...
            && let Some(sensors) = sensors.as_ref()
        {
            ...
            let _ = state.state_tx.send(proto::RobotState {
                ...
                motor_velocities: sensors.velocities,
                ...
                motor_control_error: state.power_error.load_full()
                    .or_else(|| state.motor_control_error.load_full())
                    .map(|error| error.as_ref().clone()),
                home_position_reached: bringup == Bringup::Ready
                    && configured_motors_at_home(sensors, &home),
                operator_event: state
                    .operator_event
                    .load_full()
                    .map(|event| event.as_ref().clone()),
                imu_rpy: sensors.attitude_rpy,
...
fn configured_motors_at_home(sensors: &duck_control::Sensors, home: &[f64; NUM_JOINTS]) -> bool {
    let mut count = 0;
...
fn apply_intent(state: &RobotState, intents: &Intents, call: &proto::Call) -> bool {
    ...
    match call {
        ...
        proto::Call::RobotSound(p) => {
            ...
            let publish = if p.tag == proto::SoundTag::Wheee {
                let active = p.hold.unwrap_or(true);
                state.wheee_reported.swap(active, Ordering::Relaxed) != active
            } else {
                true
            };
            if publish {
                let action = if p.tag == proto::SoundTag::Wheee && p.hold == Some(false) {
                    "wheee_stop"
                } else {
                    p.tag.as_str()
                };
                state.publish_operator_event(
                    "sound",
                    action,
...
fn dispatch(
    ...
    match call {
        ...
        proto::Call::RobotDo(p) => {
            ...
            state.publish_operator_event("skill", p.skill.as_str(), &result);
        ...
        proto::Call::RobotSound(p) => {
            ...
            if p.tag == proto::SoundTag::Wheee {
                state
                    .wheee_reported
                    .store(p.hold.unwrap_or(true), Ordering::Relaxed);
            }
            let action = if p.tag == proto::SoundTag::Wheee && p.hold == Some(false) {
                "wheee_stop"
            } else {
            ...
            state.publish_operator_event("sound", action, &result);
            proto::Response::ok(Some(id), &result)
        }
        ...
        proto::Call::RobotShutdown => {
            ...
            let result = proto::IntentResult::accepted();
            state.publish_operator_event("posture", "shutdown", &result);
            proto::Response::ok(Some(id), &result)
        }
        ...
        proto::Call::RobotStop => {
            intents.stop();
            let result = proto::IntentResult::accepted();
            state.publish_operator_event("posture", "stop", &result);
            proto::Response::ok(Some(id), &result)
        }
        ...
        proto::Call::RobotEnable(p) => {
            ...
            let result = if on && let Some(reason) = state.motor_control_error.load_full() {
                proto::IntentResult::refused(reason.as_str())
            } else {
                ...
                if on { state.power_error.store(None); }
            ...
            state.publish_operator_event(
                "posture",
                if on { "policy_on" } else { "policy_off" },
                &result,
            );
            proto::Response::ok(Some(id), &result)
        }
        ...
        proto::Call::RobotModels(params) => {
            match state.models.request(params.clone()) {
                Ok(status) => proto::Response::ok(Some(id), &status),
                Err(error) => proto::Response::err(Some(id), proto::Error::new(proto::code::INVALID_PARAMS, error)),
            }
        }
        proto::Call::RobotCalibration(params) => {
            match state.calibration.request(params) {
                Ok(status) => proto::Response::ok(Some(id), &status),
                Err(error) => proto::Response::err(Some(id),
                    proto::Error::new(proto::code::INVALID_PARAMS, error)),
            }
        }
        proto::Call::RobotInit => {
            let result = if let Some(reason) = state.motor_control_error.load_full() {
                proto::IntentResult::refused(reason.as_str())
            } else {
                ...
                state.power_error.store(None);
                intents.request_init();
                proto::IntentResult::accepted()
            };
            state.publish_operator_event("posture", "init", &result);
            proto::Response::ok(Some(id), &result)
        }
        ...
        proto::Call::RobotRelax => {
            ...
            intents.request_relax();
            let result = proto::IntentResult::accepted();
            state.publish_operator_event("posture", "relax", &result);
            proto::Response::ok(Some(id), &result)
        }
...
mod tests {
    ...
    #[test]
    fn blocked_motor_transport_refuses_start_but_allows_disable_and_relax() {
        let s = state();
        s.motor_control_error.store(Some(Arc::new("固件不兼容".to_owned())));
        let intents = Intents::new();
        for call in [
            proto::Call::RobotInit,
            proto::Call::RobotEnable(proto::EnableParams { on: true, toggle: false }),
            proto::Call::RobotEnable(proto::EnableParams { on: true, toggle: true }),
        ] {
            let result: proto::IntentResult = dispatch(&s, &intents, proto::Id::Number(1), &call)
                .result_as().unwrap();
        ...
        let off: proto::IntentResult = dispatch(&s, &intents, proto::Id::Number(2),
            &proto::Call::RobotEnable(proto::EnableParams { on: true, toggle: true }))
            .result_as().unwrap();
        assert!(off.accepted);
        assert!(!intents.enabled());
        let relax: proto::IntentResult = dispatch(&s, &intents, proto::Id::Number(3),
            &proto::Call::RobotRelax).result_as().unwrap();
        assert!(relax.accepted);
        assert!(matches!(intents.take_power_request(), Some(intents::PowerRequest::Relax)));
    }
    ...
    /// Discrete events are retained for another subscriber (the web page watching the gamepad),
    /// while a held sound publishes only its edges rather than one event per control tick.
    #[test]
    fn operator_events_cross_clients_and_wheee_is_edge_triggered() {
        let mut params = Params::default();
        params.audio.enabled = true;
        let state = RobotState::new(&params, false, false);
        let intents = Intents::new();

        let _: proto::IntentResult = dispatch(
            &state,
            &intents,
            proto::Id::Number(1),
            &proto::Call::RobotInit,
        ...
        .result_as()
        .unwrap();
        let init = state.operator_event.load_full().expect("init event");
        assert_eq!((init.sequence, init.kind.as_str(), init.action.as_str()), (1, "posture", "init"));
        assert!(init.accepted);

        let held = proto::Call::RobotSound(proto::SoundParams {
            tag: proto::SoundTag::Wheee,
            hold: Some(true),
        });
        assert!(apply_intent(&state, &intents, &held));
        let started = state.operator_event.load_full().expect("wheee start");
        assert_eq!((started.sequence, started.action.as_str()), (2, "wheee"));

        assert!(apply_intent(&state, &intents, &held));
        assert_eq!(
            state.operator_event.load_full().expect("same event").sequence,
            2,
            "a repeated hold is not a new operator event"
        );
        ...
        let stopped = state.operator_event.load_full().expect("wheee stop");
        assert_eq!((stopped.sequence, stopped.action.as_str()), (3, "wheee_stop"));
    }
    ...
    fn health_names_the_bus_while_waiting_for_it() {
        ...
        let reason = health.reason.unwrap();
        assert!(
            reason.contains("motor/IMU gateway") && reason.contains("/dev/ttyACM0"),
    ...
    #[test]
    fn a_disconnected_running_bus_is_degraded_and_safe() {
        let s = RobotState::new(&Params::default(), false, false);
        s.ticks.store(20, Ordering::Relaxed);
        s.startup_bus_failures.store(3, Ordering::Relaxed);
        s.consecutive_errors
            .store(s.max_consecutive_errors, Ordering::Relaxed);

        let health = s.health();
        assert!(!health.healthy);
        assert!(health.degraded, "a cable fault must not roll back a release");
        let reason = health.reason.expect("a reconnecting bus has a reason");
        assert!(reason.contains("reconnecting") && reason.contains("safe state"));
    }

    /// A live handle that starts returning errors must be discarded. Keeping it and retrying
    /// reads forever cannot recover after Linux re-enumerates the CDC device on reinsertion.
    #[tokio::test]
    async fn a_lost_stm32_link_clears_motion_and_returns_for_reopen() {
        struct DisconnectingIo {
            inner: FakeIo,
            reads_left: u32,
        }

        impl RobotIo for DisconnectingIo {
            fn read(&mut self) -> duck_control::io::Result<duck_control::Sensors> {
                if self.reads_left == 0 {
                    return Err(duck_control::IoError::Simulated);
                }
                self.reads_left -= 1;
                self.inner.read()
            }
        ...
        let mut params = Params::default();
        params.audio.enabled = false;
        params.update_gate.max_consecutive_errors = COAST_TICKS + 1;
        ...
        let exit = tokio::time::timeout(
            ...
            control_loop(
                DisconnectingIo {
                    ...
                    // One read adopts the startup pose; the next tick proves the handle was live.
                    reads_left: 2,
                },
                Arc::clone(&state),
                Arc::clone(&intents),
        ...
        .await
        .expect("a failed handle must not be retried forever");

        assert_eq!(exit, ControlLoopExit::BusLost);
        assert!(!intents.enabled());
        assert_eq!(intents.snapshot().command.twist, [0.0; 3]);
        assert!(!state.moving.load(Ordering::Relaxed));
        assert!(!state.homed.load(Ordering::Relaxed));
        assert!(!state.imu_ready.load(Ordering::Relaxed));
        assert!(state.startup_bus_failures.load(Ordering::Relaxed) > 0);
    }
    ...
    async fn control_loop_probe_with<T: RobotIo>(
        ...
        impl<T: RobotIo> RobotIo for Borrowed<'_, T> {
            fn set_position_limits(&mut self, limits: &[duck_control::io::MotorPositionLimit]) -> duck_control::io::Result<()> { self.0.set_position_limits(limits) }
            fn mark_motor_zero(&mut self, id: u8) -> duck_control::io::Result<()> { self.0.mark_motor_zero(id) }
            fn motor_torque_enabled(&self) -> Option<bool> { self.0.motor_torque_enabled() }
            fn motor_control_error(&self) -> Option<&'static str> { self.0.motor_control_error() }
    ...
    async fn robot_init_powers_the_joints_and_ramps_home() {
        let mut resting = DEFAULT_POSITION;
        // Deliberately far enough from home to satisfy the seated-boot detector. Explicit init
        // must still interpolate; seated posture is relevant only when the policy is enabled.
        for joint in [0, 1, 2, 3, 4, 10, 11, 12, 13, 14] {
            resting[joint] = DEFAULT_POSITION[joint] + 0.6;
        }
    ...
    #[tokio::test]
    async fn init_ramps_toward_the_saved_pose_instead_of_the_motor_zero() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("calibration.json");
        let mut config = calibration::Config::default();
        config.home[0] = 1.0;
        std::fs::write(&path, serde_json::to_vec(&config).unwrap()).unwrap();
        let mut params = Params::default();
        params.policy.enabled = false;
        let state = Arc::new(RobotState::new(&params, false, false));
        state.calibration.load(path);
        let intents = Arc::new(Intents::new());
        intents.request_init();
        let loop_state = state.clone();
        let task = tokio::spawn(async move {
            let mut io = FakeIo::at(DEFAULT_POSITION).frozen();
            control_loop_probe_with(&mut io, loop_state, intents, params, Duration::from_millis(2)).await;
            io
        });
        while state.ticks.load(Ordering::Relaxed) < 5 {
            tokio::time::sleep(Duration::from_millis(2)).await;
        }
        state.shutdown.store(true, Ordering::Relaxed);
        let io = task.await.unwrap();
        assert_eq!(io.torque, Some(true));
        let target = io.last_written.unwrap().positions[0];
        assert!(target > DEFAULT_POSITION[0] && target < 1.0);
        let ramp = Bringup::Homing { from: DEFAULT_POSITION, since: Instant::now() };
        if let Bringup::Homing { since, .. } = ramp {
            let midpoint = ramp.homing_target(since + HOME_RAMP / 2, &config.home).unwrap();
            assert!((midpoint[0] - (DEFAULT_POSITION[0] + 1.0) / 2.0).abs() < 1e-9);
        }
    }

    #[test]
    fn home_confirmation_requires_actual_fresh_enabled_motor_feedback() {
        let mut sensors = FakeIo::at(DEFAULT_POSITION).read().unwrap();
        assert!(!configured_motors_at_home(&sensors, &DEFAULT_POSITION));
        sensors.motor_flags[0] = 7;
        assert!(configured_motors_at_home(&sensors, &DEFAULT_POSITION));
        sensors.positions[0] += 0.2;
        assert!(!configured_motors_at_home(&sensors, &DEFAULT_POSITION));
        sensors.positions[0] = DEFAULT_POSITION[0];
        sensors.motor_feedback_age_ms[0] = 501;
        assert!(!configured_motors_at_home(&sensors, &DEFAULT_POSITION));
        sensors.motor_feedback_age_ms[0] = 0;
        sensors.motor_flags[0] = 3;
        assert!(!configured_motors_at_home(&sensors, &DEFAULT_POSITION));
    ...
    #[tokio::test]
    async fn rejected_init_is_published_in_state_and_does_not_retry() {
        struct RejectEnable { io: FakeIo, attempts: usize }
        impl RobotIo for RejectEnable {
            fn set_position_limits(&mut self, limits: &[duck_control::io::MotorPositionLimit]) -> duck_control::io::Result<()> { self.io.set_position_limits(limits) }
            fn read(&mut self) -> duck_control::io::Result<duck_control::Sensors> { self.io.read() }
            fn write(&mut self, t: &duck_control::JointTargets) -> duck_control::io::Result<()> { self.io.write(t) }
            fn set_gain(&mut self, kp: u16) -> duck_control::io::Result<()> { self.io.set_gain(kp) }
            fn slow_sensors(&mut self) -> duck_control::io::Result<duck_control::SlowSensors> { self.io.slow_sensors() }
            fn set_torque(&mut self, on: bool) -> duck_control::io::Result<()> {
                if on {
                    self.attempts += 1;
                    Err(duck_control::io::IoError::Bus("CAN 发送失败或队列已满".into()))
                } else { self.io.set_torque(false) }
            }
        }
        let mut params = Params::default();
        params.policy.enabled = false;
        ...
        let loop_intents = Arc::clone(&intents);
        let handle = tokio::spawn(async move {
            let mut io = RejectEnable { io: FakeIo::at(DEFAULT_POSITION), attempts: 0 };
            control_loop_probe_with(&mut io, loop_state, loop_intents, params, Duration::from_millis(2)).await;
            io.attempts
        });
        let received = tokio::time::timeout(Duration::from_secs(2), async {
            loop {
                let frame = states.recv().await.unwrap();
                if frame.motor_control_error.is_some() { break frame; }
            }
        }).await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        s.shutdown.store(true, Ordering::Relaxed);
        let attempts = handle.await.unwrap();
        let frame = received.expect("queued init failure was never published");
        assert!(frame.motor_control_error.unwrap().contains("CAN 发送失败"));
        assert!(!frame.home_position_reached);
        assert!(!intents.enabled());
        assert_eq!(attempts, 1);
    }

    /// A seated `robot.init` stays on the linear homing path even when sitstand is loaded.
    ///
    /// The bundled networks are loaded to make the tempting policy path available, but the
    /// policy switch remains off and no `rise` frame may be produced. FakeIo never touches a
    /// physical bus.
    #[tokio::test]
    #[ignore = "requires ONNX Runtime and bundled policies; run on the build board"]
    async fn seated_init_is_linear_even_with_sitstand_loaded() {
        struct WatchdogIo {
            io: FakeIo,
            enabled_at: Option<Instant>,
            first_command_us: Arc<AtomicU64>,
        }

        impl RobotIo for WatchdogIo {
            fn set_position_limits(&mut self, limits: &[duck_control::io::MotorPositionLimit]) -> duck_control::io::Result<()> {
            ...
            fn write(
                ...
                if let Some(enabled_at) = self.enabled_at.take() {
                    self.first_command_us.store(
                        enabled_at.elapsed().as_micros() as u64,
                        Ordering::Relaxed,
                    );
                }
        ...
        for cancel in [false, true] {
            let models = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../policies");
            let mut params = Params::default();
            params.audio.enabled = false;
            params.policy.enabled = true;
            params.policy.walk = Some(models.join("alpha_walking.onnx"));
            params.policy.stand = Some(models.join("alpha_stand.onnx"));
            params.policy.sitstand = Some(models.join("alpha_sitstand.onnx"));
            params.policy.ground_pick = Some(PathBuf::from("none"));
            params.policy.kick_left = Some(PathBuf::from("none"));
            params.policy.kick_right = Some(PathBuf::from("none"));
            params.policy.roulade = Some(PathBuf::from("none"));
            params.safety.limp_fall = false;
            let mut sitting = DEFAULT_POSITION;
            for joint in [0, 1, 2, 3, 4, 10, 11, 12, 13, 14] {
                sitting[joint] += 0.6;
            }
            let s = Arc::new(RobotState::new(&params, false, false));
            let intents = Arc::new(Intents::new());
            let mut states = s.state_tx.subscribe();
            let first_command_us = Arc::new(AtomicU64::new(u64::MAX));
            intents.request_init();
            let io = WatchdogIo {
                io: FakeIo::at(sitting),
                enabled_at: None,
                first_command_us: Arc::clone(&first_command_us),
            };
            let handle = tokio::spawn(control_loop(
                io,
                Arc::clone(&s),
            ...
            let observed = tokio::time::timeout(Duration::from_secs(10), async {
                let mut saw_home_ramp = false;
                let mut requested_relax = false;
                loop {
                    let frame = states.recv().await.unwrap();
                    assert!(!intents.enabled(), "init turned on the policy switch");
                    assert!(
                        frame.motor_control_error.is_none(),
                        "{:?}",
                        frame.motor_control_error
                    );
                    assert_ne!(
                        frame.policy, "rise",
                        "robot.init must not execute the sitstand policy"
                    );
                    saw_home_ramp |= frame.policy == "homing";
                    if cancel && saw_home_ramp && !requested_relax {
                        intents.request_relax();
                        requested_relax = true;
                    }
                    if requested_relax
                        && frame.policy == "held"
            ...
            handle.await.unwrap();
            assert!(
                s.policy_error.load_full().is_none(),
                "{:?}",
                s.policy_error.load_full()
            );
            assert!(
                first_command_us.load(Ordering::Relaxed) < 100_000,
                "first command after torque-on missed STM32's 100 ms watchdog: {} us",
                first_command_us.load(Ordering::Relaxed),
            );
            let frame = observed.expect("seated init never progressed homing -> held");
            if cancel {
                assert!(!s.homed.load(Ordering::Relaxed));
            } else {
                for (joint, position) in frame.joints.iter().enumerate() {
                    assert!((position - DEFAULT_POSITION[joint]).abs() < 0.1);
                }
            }
        }
    }
