...
电脑提前生成完整指令文件，上传并校验后单独启动。robotd 在本地 50 Hz 控制循环执行，Wi-Fi 不参与指令定时；断网继续执行既定任务，不重发、不自动重启任务。正常结束或 API 停止后回到默认站姿并保持初始化；反馈、总线等故障或放松/关机请求仍会失能。旧实时 acquire / command / release / feedback 接口已移除。
...
不会插值、平滑、跳帧或修改实验轨迹。若某次控制 tick 提前到达下一帧时间，保持上一帧参数并记录该周期，绝不提前执行下一帧。到任务结束时间后退出原始实验指令，改由普通初始化保持路径发送保存的默认站姿；确认归位后才释放独占，不会无限保持最后一条实验指令。Linux 调度存在抖动，记录真实时间，不承诺硬实时。
...
启动再次校验文件与当前标定。控制线程允许从两种稳定状态接管：放松且全部配置电机反馈失能，或机器人已经初始化且全部配置电机反馈使能；归位进行中不能接管。两种入口都继续检查新鲜反馈、IMU、限位、当前姿态与首帧匹配和温度。已初始化入口不会重新使能或重新归位，取得独占后普通控制的写入会在同一控制循环内被实验替代。任务运行时其他普通控制和标定不能覆盖它，原有停止/放松按钮仍可终止任务。
...
- `status.json`：任务参数、执行帧数、结束原因，以及是否确认失能或确认回到初始化站姿；包的 SHA-256 在查询响应和下载头中提供，避免自引用。
