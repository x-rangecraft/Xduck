...
**Only the alpha variant and only the Radxa.** Motor state and the primary BMI088 IMU arrive
together from STM32 over DMUSB v4; the old `imu_to_dxl` path is no longer active. Every
shipped policy is `alpha_*`. The wheeled configuration survives as one params switch —
...
`robotd` alone owns the STM32 USB CDC port. The independent IMU port remains a reserved
configuration field for a possible second IMU and is not opened:

```text
                     robotd — control thread
                              │
                              │  duck_control::bus::DynamixelIo
                              │  serialport · DMUSB v4 · 1,000,000 8N1
                              ▼
                         /dev/ttyACM0
                              │
                 ┌────────────┴────────────┐
                 │                         │
       primary BMI088 sample       14 fixed motor slots
       gyro/gravity/quaternion        CAN2/CAN3/CAN1
```

The IMU and motors are decoded from the same 344-byte state payload. V4 carries the
body-to-world w,x,y,z quaternion directly; control and odometry consume it without an RPY
round trip. The authoritative byte layout is
`10-source/rk3566/STM32_DMUSB_V4_PROTOCOL.md`.
...
On RK3566, `DynamixelIo` owns the STM32 USB CDC device named by `[bus].port`. DMUSB v4
carries both motor state and the primary BMI088 IMU in that one stream. `[bus].imu_port` is
kept as a reserved second-IMU interface but is not opened today. After sustained read
failure, `robotd` clears motion and enable intents, reports a degraded safe state, drops the
failed serial handle, and repeats open plus protocol verification until the USB device comes
back. A successful reconnect starts in limp state and never resumes motion without a new
explicit enable. Independently, STM32 disables its motor routes after 100 ms without a host
command, so physical safety does not rely on writing through a cable that has already gone.
...
The optional `operator_event` block carries the latest discrete posture, skill, or sound request,
including its sequence and accepted/refused result. It is shared state rather than a reply tied to
one socket because the gamepad and web console are separate clients: a page watching a button press
from `padd` must be able to report it. Movement, head, pose, and mouth updates stay out of the block;
they are continuous intents and would replace useful feedback up to 50 times per second. Repeated
`wheee` hold notifications are collapsed to their start and release edges for the same reason.
...
`init` never evaluates a policy: it always linearly interpolates from the latest measured pose to
the default standing pose and then holds. The seated-boot `sitstand` rise belongs exclusively to
`enable`, so an operator can establish the default pose without implicitly starting any network.
