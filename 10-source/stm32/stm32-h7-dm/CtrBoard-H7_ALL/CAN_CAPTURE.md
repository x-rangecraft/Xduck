...
采集模块只在 `H7DM_CAN_CAPTURE=ON` 时编入固件。默认选中电机
...
cmake -S 10-source/stm32/stm32-h7-dm/CtrBoard-H7_ALL \
  -B 20-build/stm32/stm32-h7-dm/can-capture \
  -DCMAKE_TOOLCHAIN_FILE=cmake/arm-none-eabi-gcc.cmake \
  -DCMAKE_BUILD_TYPE=Release -DH7DM_CAN_CAPTURE=ON -DH7DM_BENCH_ID1_ONLY=OFF
cmake --build 20-build/stm32/stm32-h7-dm/can-capture
```

已补齐 `USB_DEVICE/Target/usbd_conf.c` 和 `usbd_conf.h`，并完成整机编译。
2026-09-17 已将三电机采集镜像烧录到 STM32H723 并通过写后校验；匹配的 ELF 保存在
`30-artifacts/stm32/stm32-h7-dm/can-capture-2026-09-17-r2/`。采集脚本不烧录、
...
python3 60-tools/capture-stm32-can.py --motors 2,7,12 --duration-ms 3000 \
  --elf 30-artifacts/stm32/stm32-h7-dm/can-capture-2026-09-17-r2/CtrBoard-H7_ALL.elf
```
