...
`probe_dm_j4340_canable.py`。脚本向 `0x7FF` 发送 DLC=4 的寄存器读取帧；失能扫描会按
四种控制模式分别尝试 `ID`、`0x100+ID`、`0x200+ID`、`0x300+ID`。

`probe_dm_j4340.py` 采用官方 U2CAN Python 例程的串口封装格式：USB 串口波特率为
921600，CAN 侧应预先配置为经典 CAN 2.0 标准帧、1 Mbps。脚本只发送寄存器读取帧，
不会使能电机、写参数、保存参数或设置零点，因此不会主动让电机转动。
...
- 电机 CAN 波特率为 1 Mbps；
- USB 转 CAN 是达妙串口型 U2CAN。如果是 USB2CANFD 或其他品牌适配器，需要换传输后端。

创建环境并安装唯一依赖：

```bash
cd /Users/mac/xinchen/MicriDuck/10-source/tools/motor/dm-j4340-comm
...
```bash
.venv/bin/python probe_dm_j4340_canable.py --list-adapters
```

只读探测默认 ID 1、2、3：

```bash
.venv/bin/python probe_dm_j4340_canable.py --ids 1,2,3
```

不知道 CAN ID 时，可以安全地只读扫描手册规定的完整范围：

```bash
.venv/bin/python probe_dm_j4340_canable.py --ids 1-31
```

达妙串口型 U2CAN 才使用以下命令：

```bash
.venv/bin/python probe_dm_j4340.py --list-ports
.venv/bin/python probe_dm_j4340.py --port /dev/cu.usbmodem1101 --ids 1-15
```

成功时每台电机会显示 `ONLINE`，并列出反馈 CAN ID、控制模式、映射范围、母线电压和
温度。`no reply` 通常依次检查：串口设备名、适配器类型、CAN 侧 1 Mbps、CAN-H/CAN-L、
终端电阻、电机 ID、以及是否有重复 Master ID。

机器可读输出：

```bash
.venv/bin/python probe_dm_j4340_canable.py --ids 1,2,3 --json
```
