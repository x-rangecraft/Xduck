...
from i2rt_protocol import Register
...
class CanableProtocolTest(unittest.TestCase):
    ...
    def test_read_request(self):
        message = build_register_request(0x13, 0x15)
        self.assertEqual(message.arbitration_id, 0x7FF)
        self.assertFalse(message.is_extended_id)
        self.assertEqual(bytes(message.data), bytes([0x13, 0x00, 0x33, 0x15]))

    def test_float_reply(self):
        payload = bytes([2, 0, 0x33, 0x15]) + struct.pack("<f", 12.5)
        message = can.Message(arbitration_id=0x12, data=payload, is_extended_id=False)
        result = decode_register_message(message, 2, Register(0x15, "pmax", "f32"))
        self.assertIsNotNone(result)
        assert result is not None
        value, raw = result
        self.assertAlmostEqual(value, 12.5)
        self.assertEqual(raw, "012#0200331500004841")
